...
export class Slack {
    ...
    private async _request<R extends ISlackResponse>(
        ...
        if (!data.ok) {
            throw new Error(
                `Slack API error: ${JSON.stringify(data, null, 2)}`,
            );
        }
    ...
    async oAuth(params: QueryParams) {
        return await this._request<ISlackOAuthResponse>(
            "GET",
            "/oauth.v2.access",
            {
                params,
    ...
    async postMessage(body: SlackMessage) {
        return this._request<ISlackMessageResponse>(
            "POST",
            "/chat.postMessage",
            {
                body,
    ...
    async publishView(body: SlackHomeView) {
        return this._request<ISlackResponse>("POST", "/views.publish", {
            body,
        });
    }
